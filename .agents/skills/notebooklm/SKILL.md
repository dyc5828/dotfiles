...
Before any `notebooklm` command, verify the CLI is on PATH:

```bash
notebooklm --version
```

If you get `command not found`, the Python CLI hasn't been installed on this
machine yet. **Use the `notebooklm-setup` skill first** — it installs
`notebooklm-py`, Playwright Chromium, and walks through Google OAuth. Then
return to this skill.

(This Setup check section is a dotfiles-only addition; it is not in the upstream
SKILL.md and will be removed if `notebooklm skill install` is ever run. See the
`notebooklm-setup` skill for the dotfiles refresh procedure.)

## Installation

This SKILL.md ships via dotfiles; the `notebooklm-py` Python CLI and Google auth are installed by the **`notebooklm-setup` skill** — see the **Setup check** section above.

**Do not run `notebooklm skill install`** in this dotfiles-managed setup — it would overwrite this file with the upstream version, dropping the Setup check pointer back to `notebooklm-setup` and reinstating contradictory upstream install guidance.

(For non-dotfiles users, upstream installation instructions live at <https://github.com/teng-lin/notebooklm-py>.)
